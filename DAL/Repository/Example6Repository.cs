...
{
    public class Example6Repository
    {
        private readonly string _connectionString;
        public Example6Repository(string connectionString)
        {
            _connectionString = connectionString;
        }

        public object GetSingleOrDefaultAddress()
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                return connection.QuerySingleOrDefault<AddressDTO>("SELECT * FROM Address");
            }
        }
